...
def fit_learnable_M(X, t, M_max=5, lr=0.001, batch_size= 10, epochs=100):
    """
    Train a logistic regression model with a learnable polynomial degree using stochastic gradient descent (SGD).
    ...
    M_max : int, optional
        Maximum polynomial degree to consider. Default is 5.
    lr : float, optional
    ...
    Returns:
    alpha : np.ndarray
        Softmax-normalized weights indicating the importance of different polynomial degrees.
    W : dict
        Dictionary containing trained weight parameters for each polynomial degree M.
    """
    n = X.shape[0]
    D = X.shape[1]
