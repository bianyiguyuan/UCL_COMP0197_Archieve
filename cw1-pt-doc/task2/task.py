...
def random_guess(test_loader, num_classes=10):
    """
    Simulates a random guessing strategy in multiclass classification.
    ...
    --------
    None
        Prints the accuracy of the random guessing strategy.
    """
    correct = 0
    total = 0
...
def basic_elm(train_loader, num_classes=10, lr=0.01, epochs=10):
    """
    Trains a basic Extreme Learning Machine (ELM) model using Stochastic Gradient Descent (SGD).
...
def mixed_elm(train_loader, is_mixup=False, is_ensemble=False, n_models=5):
    """
    Trains an Extreme Learning Machine (ELM) model with optional MixUp augmentation and ensemble learning.

    Parameters:
    -----------
    train_loader : DataLoader
        The DataLoader containing the training dataset.
    is_mixup : bool, optional
        Whether to apply MixUp augmentation (default is False).
    is_ensemble : bool, optional
        Whether to use an ensemble of multiple ELM models (default is False).
    n_models : int, optional
        Number of models in the ensemble (default is 5).

    Returns:
    --------
    MyExtremeLearningMachine or MyEnsembleELM
        Trained ELM model.
    """
    if is_ensemble:
        model = MyEnsembleELM(
